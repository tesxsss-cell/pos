...
@section('content')
    ...
    {{-- Tampilan kartu khusus HP agar tombol scan tidak tersembunyi di sisi kanan tabel. --}}
        ...
        @forelse ($rows as $baris)
            ...
            <article class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <h2 class="truncate text-sm font-semibold text-slate-900">{{ $baris['name'] }}</h2>
                        <p class="mt-0.5 text-xs text-slate-500">{{ $baris['sku'] }} &middot; {{ $baris['unit'] }}</p>
                    </div>
                    <span class="shrink-0 text-sm font-semibold text-slate-900">Rp {{ number_format((float) $baris['base_price'], 0, ',', '.') }}</span>
                </div>

                <div class="mt-3 grid grid-cols-2 gap-2 rounded-lg bg-slate-50 p-3 text-xs">
                    <div>
                        <span class="text-slate-500">Stok</span>
                        <p class="font-medium text-slate-800">{{ $baris['stock'] }} {{ $baris['unit'] }}</p>
                    </div>
                    <div>
                        <span class="text-slate-500">Barcode</span>
                        <div id="kode-mobile-{{ $baris['id'] }}" class="mt-0.5 flex flex-wrap gap-1">
                            @forelse ($baris['barcodes'] as $bc)
                                <span class="rounded bg-white px-2 py-0.5 font-medium text-slate-700">{{ $bc['code'] }}</span>
                            @empty
                                <span class="teks-kosong text-slate-400">Belum ada</span>
                            @endforelse
                        </div>
                    </div>
                </div>
                ...
                <button type="button"
                        data-id="{{ $baris['id'] }}"
                        class="tombol-barcode mt-3 min-h-11 w-full rounded-md px-4 py-2.5 text-sm font-semibold text-white {{ $punya ? 'bg-amber-600 active:bg-amber-700' : 'bg-indigo-600 active:bg-indigo-700' }}">
                    {{ $punya ? 'Ganti barcode' : 'Daftarkan barcode' }}
                </button>
            </article>
        @empty
            <div class="rounded-xl bg-white px-4 py-8 text-center text-sm text-slate-400 shadow">Tidak ada produk untuk cabang ini.</div>
        @endforelse
    </div>
    ...
    <style>
        /* Kotak kamera responsif dan tetap tanpa animasi pada layar HP. */
        #popup-scanner .bcs-kotak { min-height: 220px; max-height: 42dvh; }
        @media (max-width: 640px) {
            #popup-scanner .bcs-kotak { min-height: 210px; max-height: 38dvh; aspect-ratio: 4 / 3; }
            #popup-scanner .bcs-aksi > button,
            #popup-scanner .bcs-aksi > select { min-height: 44px; }
        }
    </style>
    <div id="popup-barcode" class="fixed inset-0 z-50 hidden items-end justify-center overflow-y-auto bg-slate-900/70 p-0 sm:items-center sm:p-4">
        <div class="mt-auto max-h-[96dvh] w-full max-w-lg overscroll-contain overflow-y-auto rounded-t-2xl bg-white p-4 shadow-xl sm:my-auto sm:max-h-[92vh] sm:rounded-2xl sm:p-5">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <h2 id="popup-judul" class="text-base font-semibold text-slate-900">Daftarkan barcode</h2>
                    <p class="text-xs text-slate-500">Arahkan barcode ke kamera HP. Kode akan terisi otomatis dan masih bisa diedit.</p>
                </div>
                <button type="button" id="popup-tutup" class="text-2xl leading-none text-slate-400 hover:text-slate-600">&times;</button>
            </div>

            {{-- Kotak kamera responsif tanpa animasi (diisi oleh BarcodeScanner.pasang). --}}
            <div id="popup-scanner" class="mt-3 w-full"></div>
            <p class="mt-1 text-[11px] leading-relaxed text-slate-400">
                Di HP, buka aplikasi melalui HTTPS dan izinkan kamera. Pilih kamera belakang bila tersedia.
            </p>
    ...
    {{-- Scanner daftar produk di-inline agar tersedia bersama halaman saat online maupun offline. --}}
    <script>
/**
 * offline-barcode.js
    ...
    <script>
        (function () {
            ...
            function bukaPopup(produk, tombol) {
                ...
                overflowBodySebelum = document.body.style.overflow;
                ...
                // Jangan membuka keyboard virtual otomatis di HP karena dapat
                // menutupi tampilan kamera. Autofocus hanya untuk mouse/laptop.
            ...
            function tutupPopup() {
                ...
                document.body.style.overflow = overflowBodySebelum;
                produkAktif = null;
                tombolAktif = null;
            }
            ...
            function tandaiTerdaftar(produk, kode) {
                ...
                ['kode-' + produk.id, 'kode-mobile-' + produk.id].forEach(function (idKotak) {
                    var kotak = document.getElementById(idKotak);
                    if (! kotak) { return; }
